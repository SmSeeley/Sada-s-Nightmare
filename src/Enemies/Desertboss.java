...
public class Desertboss extends Enemy {
   ...
   @Override
    ...
    private void relocate() {
        try {
            // Ensure the new location is within the map bounds and is a valid tile
            int mapWidth = currentMap.getWidth();
            int mapHeight = currentMap.getHeight();
            Point newLocation = null;

            // Attempt to find a valid tile
            for (int attempts = 0; attempts < 10; attempts++) { // Limit attempts to avoid infinite loops
                ...
                // Get the map tile at the random coordinates
            ...
            // If a valid location was found, update the boss's position
            if (newLocation != null) {
    ...
    public void chase(Sada sada) {
        ...
        float dx = sadaX - desertX;
        float dy = sadaY - desertY;
