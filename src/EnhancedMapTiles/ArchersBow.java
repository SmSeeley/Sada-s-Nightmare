package EnhancedMapTiles;

import Builders.FrameBuilder;
import Engine.AudioPlayer;
import Engine.ImageLoader;
import GameObject.Frame;
import GameObject.GameObject;
import GameObject.SpriteSheet;
import Level.EnhancedMapTile;
import Level.Player;
import Level.ScriptState;
import Level.TileType;
import ScriptActions.TextboxScriptAction;
import Utils.Point;

/**
 * Enhanced map tile for a weapon pickup (Archer's Bow).
 * - Passable prop that renders the bow sprite.
 * - On collision: collects, equips, shows a textbox, and plays a sound.
 * - Can be purchased from shopkeeper or found as pickup.
 */
public class ArchersBow extends EnhancedMapTile {
    private Frame bowFrame;
    private GameObject bowObject;
    private boolean collected = false;

    private static final java.util.HashSet<String> collectedBows = new java.util.HashSet<>();
    private static boolean hasBow = false;
    private static int bowDamage = 4; // Higher damage than sword
    private static int bowRange = 150; // Bow has range advantage

    // Textbox handling
    private TextboxScriptAction activeTextbox = null;
    private boolean textboxSetupDone = false;
    private int textboxTimer = 0;

    public ArchersBow(Point location) {
        super(location.x, location.y,
                new SpriteSheet(ImageLoader.load("archersbow.png"), 16, 16),
                TileType.PASSABLE);
    }

    private String key() {
        return "ArchersBow@" + x + "," + y;
    }

    public static boolean isCollectedAt(float x, float y) {
        return collectedBows.contains("ArchersBow@" + x + "," + y);
    }

    public static boolean isCollectedAt(Point p) {
        return isCollectedAt(p.x, p.y);
    }

    @Override
    protected GameObject loadBottomLayer(SpriteSheet sheet) {
        bowFrame = new FrameBuilder(sheet.getSubImage(0, 0))
                .withScale(3)
                .withBounds(0, 0, 16, 16)
                .build();

        bowObject = new GameObject(x, y - 16, bowFrame);
        return bowObject;
    }

    @Override
    public void update(Player player) {
        // If a textbox is active, drive its lifecycle every frame
        if (activeTextbox != null) {
            if (!textboxSetupDone) {
                try {
                    activeTextbox.setMap(map);
                    activeTextbox.setup();
                    textboxSetupDone = true;
                    textboxTimer = 0;
                } catch (Exception e) {
                    System.out.println("[ArchersBow] Textbox setup failed: " + e.getMessage());
                    activeTextbox = null;
                }
            } else {
                ScriptState state = ScriptState.RUNNING;
                try {
                    state = activeTextbox.execute();
                } catch (Exception ignored) {}

                textboxTimer++;
                if (state == ScriptState.COMPLETED || textboxTimer >= 120) {
                    try {
                        activeTextbox.cleanup();
                    } catch (Exception ignored) {}
                    activeTextbox = null;
                    textboxSetupDone = false;
                    textboxTimer = 0;
                }
            }
        }

        if (collected || bowObject == null) return;

        // Detect pickup
        if (player.getBounds().intersects(bowObject.getBounds())) {
            collected = true;
            collectedBows.add(key());
            hasBow = true;

            bowObject.setLocation(-100, -100);

            // Equip on player
            try {
                player.setHasBow(true);
            } catch (Exception e) {
                try {
                    java.lang.reflect.Method m = player.getClass().getMethod("equipBow");
                    m.invoke(player);
                } catch (Exception ignored) {}
            }

            // ✅ Play special pickup sound
            try {
                AudioPlayer.playSound("Resources/audio/Key_Item.wav", -3.0f); // volume around 70%
            } catch (Exception e) {
                System.out.println("[ArchersBow] Failed to play special_item sound: " + e.getMessage());
            }

            // Create textbox
            TextboxScriptAction text = new TextboxScriptAction();
            text.addText("You picked up the Archer's Bow! Perfect for ranged attacks!");
            activeTextbox = text;
            textboxSetupDone = false;
        }
    }

    // Static methods for shop integration
    public static boolean sethasBow() {
        return hasBow;
    }

    public static void setBow(boolean has) {
        hasBow = has;
    }

    public static int getBowDamage() {
        return bowDamage;
    }

    public static int getBowRange() {
        return bowRange;
    }

    // Method for shopkeeper to give bow to player without pickup animation
    public static void giveToPlayer(Player player) {
        hasBow = true;
        try {
            player.setHasBow(true);
        } catch (Exception e) {
            try {
                java.lang.reflect.Method m = player.getClass().getMethod("equipBow");
                m.invoke(player);
            } catch (Exception ignored) {}
        }
    }
}
